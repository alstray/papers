...
\begin{frame}\frametitle{$k$彩色遷移問題}
  ...
  \begin{block}{$k$彩色遷移問題}
    \begin{itemize}
      \item \structure{$k \ge 4$}のとき，
            一般に\structure{\bf PSPACE完全}となる~[Bonsma+ '09]．
...
\begin{frame}\frametitle{ASP の基本的な構文}
  ...
  \begin{itemize}
    \item \structure{論理プログラム}とは，以下の形式の
          ...
          \begin{block}{}
            \centering
            $a_0$ \code{:-} $a_1, \dots, a_m,$ \code{not} 
            $a_{m+1}, \dots,$ \code{not} $a_{n}$
          \end{block}
          \begin{itemize}
