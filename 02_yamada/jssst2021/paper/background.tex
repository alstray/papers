\section{組合せ遷移問題} \label{chap:background}
...
\begin{figure*}[t]
  ...
  \begin{tabular}{ccc}
    $t=0$ (初期状態) & & $t=1$\\
    \scalebox{0.8}{\input{tikz/start}}
    & \lw{$\Rightarrow$} & 
    \scalebox{0.8}{\input{tikz/trans1}}\\
    & & $\Downarrow$\\
    & & \\
    \scalebox{0.8}{\input{tikz/trans2}}
    & \lw{$\Leftarrow$} &
    \scalebox{0.8}{\input{tikz/goal}}\\
    $t=3$ (目標状態) & & $t=2$
  \end{tabular}
  \caption{遷移系列の例}
...
\end{itemize}
しかしながら，汎用的かつ効率的なアルゴリズムはいまだ発見されていない．
