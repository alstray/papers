...
\item \structure{インクリメンタルASP解法を用いた改良アルゴリズム}
  ...
  \item 長所: 学習節が再利用が可能．ASPシステムの起動は1回のみ
...
\end{columns}
\begin{itemize}
\item \code{start(X,C)} は，スタート状態で頂点\code{X}が色\code{C}で
  塗られることを意味する．
  \code{goal(X,C)}も同様．
...
\only<3>{
\begin{itemize}
  \item 5--7行目: グラフ点彩色問題の制約を表すルールである．
  \item \code{color(X,C,T)}は，ステップ\code{T}で頂点\code{X}が色
    \code{C}で塗られることを意味する．
  \item 5行目: 各ステップ\code{T}，各頂点\code{X}に対して，
    \code{X}がただ1つの色で塗られることを個数制約を用いて表している．
  \item 6--7行目: 各ステップ\code{T}，各辺\code{edge(X,Y)}に対して，
    隣接する頂点\code{X}と\code{Y}が同じ色で塗られないことを表している．
\end{itemize}
}
\only<4>{
  \begin{itemize}
  \item 9--10行目: 遷移制約$T(\bm{x}^{t-1},\bm{x}^{t})$を表す．
  \item アトム\code{changed(X,T)}は，ステップ$T-1$とステップ$T$の間い
    で，頂点$X$の色が変化したことを意味する．
...
\begin{frame}\frametitle{実験結果: 解けた問題数}
  ...
  \item 改良ソルバーの\code{unchanged}符号化は，他が解いた問題をすべて解いていた．
