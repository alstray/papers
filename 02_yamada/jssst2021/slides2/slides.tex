...
\item インクリメンタルASP解法を用いた改良アルゴリズム
  ...
  \item ステップ数$t$を増加させながら，
    $t$に関する制約を逐次追加しインクリメンタルに解く．
  \item 長所: 効率的な解探索が可能である．
  \item 短所: デバックを行いにくい．
  \end{itemize}
\end{enumerate}
\end{frame}
...
\only<3>{
\begin{itemize}
  \item 1行目は，色数とステップ数をファクトで表している．
  \item \code{length}はステップ長$\ell$に対応している．
  \item \code{c}と\code{length}は定数であり，
    実行時にオプションから与えられる．
  \item 3行目は制約$S$に対応するルールを，
    一貫性制約を用いて表している．
\end{itemize}
}
\only<4>{
\begin{itemize}
  \item 5--7行目のルールは制約$C$に対応している．
  \item 5行目では，頂点\code{X}が色\code{C}で塗られることを意味する
    \code{color/2}を導入している．
  \item 個数制約を用いて，1つの色で塗られることを表している．
  \item 6--7行目では，個数制約を用いて，辺で結ばれた2つの頂点のうち，
...
\only<5>{
\begin{itemize}
  \item 9--10行目のルールは制約$T$に対応している．
  \item 9行目では，ステップ数$T-1$とステップ数$T$で，頂点$X$の色が変化したことを意味する
    \code{changed/2}を導入している．
  \item 10行目では，個数制約を用いて，
    色が変わる頂点は各遷移において1つであることを表している．
  \item 12行目では，制約$G$に対応するルールを，
    一貫性制約を用いて表している
\end{itemize}
}
\end{frame}
...
\begin{frame}\frametitle{まとめと今後の課題}
  ...
  \begin{block}{まとめ}
    \begin{itemize}
      ...
      \begin{itemize}
        \item 特に改良ソルバーはインクリメンタル解法に基づいた効率化を図っている．
      \end{itemize}
