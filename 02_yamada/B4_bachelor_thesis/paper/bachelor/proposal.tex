...
4行目ではアトム\code{node}によって頂点1から4を定義している．
6行目ではアトム\code{edge}によって辺を定義している．
\code{edge(X,Y)}はノード\code{X}とノード\code{Y}間に辺が存在することを意味する．
...
\code{color(X,C,T)}はステップ\code{T}において頂点\code{X}が色\code{C}で塗られることを意味する．
1行目では定数を用いて色\code{1}から色\code{c}までを定義している．
2行目では定数を用いて遷移ステップ(以下ステップ)\code{0}から\code{t}までを定義している．
...
10行目のルールは「各ステップにおいて，辺で結ばれた頂点\code{X}と\code{Y}は同じ色で塗られない」を意味する．
14～15行目は隣接関係を表したルールである．
「ステップ\code{T-1}と\code{T}($1 \geq T \geq t$)においてある2つの頂点\code{X}と\code{Y}が変化することはない」
を意味する．
