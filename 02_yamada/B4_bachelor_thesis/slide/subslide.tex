...
\begin{frame}\frametitle{$k$彩色遷移問題の性質}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item \structure{$k=2$}のとき，グラフGは2部グラフであり\structure{明らかに到達不能}．[L. Cerecedaほか '08]
...
\begin{frame}\frametitle{遷移回数$t$の導入}
  ...
  \begin{itemize}
    \item グラフ点彩色問題とその二つの実行可能解$\alpha$，$\beta$に加え， \structure{遷移回数$t$}を与える．
    \begin{itemize}
...
\begin{frame}\frametitle{グラフの調査}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item 選択基準は\structure{彩色数}が判明しているもの.[Tamuraほか '09]
    \end{itemize}
...
\begin{frame}\frametitle{判定不能インスタンスにおける最大遷移回数の比較}
