...
\begin{frame}
    ...
    \begin{flalign*}
        &\bigwedge_{1 \leq i < j \leq n} (\lnot \dE{x_i}{a} \lor \lnot \dE{x_j}{a})  \qquad  (a \in \{\ell,\ldots, u\}) \\
        &\bigvee_{i=1}^n \dE{x_i}{a}  \qquad  (a \in \{\ell,\ldots, u\}) \qquad(if\;n=u-\ell)\qquad{\color[HTML]{000C79} \bf (ALT1)} 
    \end{flalign*}
    \end{exampleblock}
    \begin{alertblock}{{\alldifferent}制約を直接符号化法で表す利点}
        \begin{itemize}
            ...
            \item {\alldifferent}制約をnot-equal制約ではなく，\structure{擬似ブール(PB)制約}で表し，直接符号化できる．
        \end{itemize}
    \end{alertblock}
\end{frame}
...
\begin{frame}
    ...
    \begin{exampleblock}{{$\Alldiff \; (x_i \in \{\ell,\dots,u\})$}の順序符号化}
        % \begin{itemize}
            % \item $x \neq y$を$x-y \leq -1 \lor y-x \leq -1$のように線形制約に置き換えてから符号化する．
            % \item $x=a$を$(\lnot \oE{x}{a} \lor \oE{x}{a-1})$ として符号化する.
        % \end{itemize}
        % \begin{flalign*}
        %     &\bigvee_{i=1}^{n} \lnot \oE{x_i}{\ell+n-2} \qquad \bigvee_{i=1}^{n} \oE{x_i}{u-n+1} \qquad {\color[HTML]{000C79} \bf (PHP)}
        % \end{flalign*}
        \begin{displaymath}
            \begin{array}{l}
