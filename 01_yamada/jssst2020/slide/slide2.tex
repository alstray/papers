...
\begin{frame}{配電網問題}
 ...
 \begin{itemize}
  ...
  \item 厳密解法として，フロンティア法を用いた解法が提案されている．
		\begin{itemize}
		 \item 実用規模の配電網問題(\structure{\textbf{スイッチ数468個}})の網羅的な列挙・
 ...
 \end{block}
  ...
  \item ヘッドが空のルールを\structure{一貫性制約}と呼ぶ．
		\begin{block}{}
		 ...
		 $ \text{:-} ~a_1, ~{\tt not}~ a_2.$
		\end{block}
		上のルールは，「$a_1$が成り立ち，$a_2$が成り立たないことは``ない''」\\
		つまり，「$a_1$と$a_2$は両方同時に成り立つ」ことを意味する．
  \item ヘッドのみのルールを\structure{ファクト}と呼ぶ．
 \end{itemize}
\end{frame}
...
\begin{frame}{根付き全域森遷移問題の解法}
 ...
 \begin{exampleblock}{シングルショット符号化}
  \begin{itemize}
   ...
   \item 各問題中の制約の大部分は共通であるため，\alert{同一の探索空間を何度も調べる}ことになり，
		 求解効率が低下するという問題点がある．
  \end{itemize}
 \end{exampleblock}
 ...
 \end{alertblock}
\begin{itemize}
 \item マルチショットASP解法を用いることで，解の遷移問題に対して，求解性能の向上が期待できる．
\end{itemize}
\end{frame}
