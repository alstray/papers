...
意味する．
なお，供給経路上，\code{S}は\code{T}の上流であることに注意が必要である．
2行目の\code{closed_switch/1}は，閉じたスイッチを表す．
%
...
%
7--8行目の\code{downstream/2}は，任意の2つのセクションの上下関係を表す．
% この\code{downstream(S,T)}は，セクション\code{S}の次にセクション\code{T}
% を経由することを意味する．
7行目のルールは，任意の2つのセクション\code{S}と\code{T}が，この順序で
接続されているならば，\code{downstream(S,T)}を生成する．
