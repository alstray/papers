...
\begin{figure}[tbp]
 \centering
 \scalebox{0.8}{\input{tikz/tikz-test-input}}
  \caption{配電網問題の例}
...
\begin{figure}[tbp]
 \centering
 \scalebox{0.8}{\input{tikz/tikz-test-output}}
...
この例は，
25箇所のセクション$S=\{s_{a},s_{b},s_{c},s_{1},\ldots, s_{22}\}$と，
16個のスイッチ$SW=\{sw_{1},\ldots, sw_{16}\}$から構成されている．
また，セクション$\{s_{a}, s_{b}, s_{c}\}$は変電所\footnote{%
本論文では，変電所に直接つながるセクションをそれぞれ変電所とする．}
を表す．

次に，図\ref{fig:test-input}の配電網問題の解を図\ref{fig:test-output}に示す．
閉じたスイッチは，$X=\{sw_{1},sw_{2},sw_{4},sw_{5},sw_{7},sw_{8},sw_{9},%
sw_{10},sw_{11},sw_{12},sw_{13},sw_{15}\}$である．
この$X$から定まる配電網の構成は，図\ref{fig:test-output}のようになり，
根付き全域森の性質を満たすことで，トポロジ制約を満たしていることがわかる．
...
まで，配電網問題の制約を満たしながら遷移していることがわかる．例として，
ステップ$t=0$から$t=1$への遷移では，スイッチ$\{sw_3,sw_5\}$がそれぞれ
切り替わっている．
