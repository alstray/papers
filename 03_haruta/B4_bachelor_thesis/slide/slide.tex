...
\begin{frame}{解集合プログラミング(Answer Set Programming; ASP)}
 ...
 \begin{alertblock}{グラフ彩色判定問題に対してASPを用いる利点}
  \begin{itemize}
   ...
   \item 充足不能コアを用いた最適化探索が可能．%%% USCを書くべきか
...
\begin{frame}{考案するASP符号化}
 \begin{block}{グラフ彩色判定問題の表現}
  ...
  各頂点を彩色するとき，必要となる最小の色数を求める問題.%%% これはグラフ彩色問題の説明では?
  \begin{itemize}
...
\begin{frame}{同色頂点数最小化問題の実験結果(最適値と最良値)}
 ...
 \begin{center}
  \input{fig/min_total.tex}%%% n を $N$　に変えてほしい もしくは 文章中の$N$を$n$にして表のnを$n$にするか
 \end{center}
...
\begin{frame}{同色頂点数最大化問題の実験結果(最適値と最良値)}
 \begin{block}{}
  \structure{問題数}: $N$次の\code{McGregor}グラフ ($3 \leq N\leq 38$: 計36問)
 \end{block}
 
 \begin{center}
  \input{fig/max_total.tex}%%% n を $N$　に変えてほしい
 \end{center}
...
\begin{frame}{多色頂点数最大化問題の実験結果(最適値と最良値)}
 \begin{block}{}
  \structure{問題数}: $N$次の\code{McGregor}グラフ ($3 \leq N\leq 15$: 計13問)
 \end{block}
 
 \begin{center}
  \input{fig/mult_total.tex}%%% n を $N$　に変えてほしい
 \end{center}
...
%%% h(n)の説明を用意しておいてほしい(コメントで)
 ...
 \begin{center}
  \input{fig/table.tex}%%% n を $N$　に変えてほしい
 \end{center}
  ...
  \item \structure{ベンチマーク問題を用いた評価実験}
        \begin{itemize}
         ...
         \item 同色頂点数最小化問題，同色頂点数最大化問題．多色頂点数最大化問題
               において，新たに最適値を発見した．%%% 合計いくつ?
         \item グラフ彩色判定問題の実行可能解の全解列挙に成功した．
        \end{itemize}
  \item \structure{グラフ彩色判定問題の実行可能解の圧縮についての調査}
        \begin{itemize}
         \item 多色頂点数最大化問題で発見した10問の最適値について
               基のグラフ彩色判定問題の実行可能解の圧縮することに成功した．%%% 12次の680億の話に置き換えましょう
        \end{itemize}
 \end{enumerate}
...
\begin{frame}[noframenumbering]{グラフ彩色問題の実行可能解の全解列挙(2/2)}
 ...
 \begin{center}
  \input{fig/enum_total.tex}%%% nを$N$に変更
 \end{center}
