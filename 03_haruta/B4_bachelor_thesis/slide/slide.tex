...
\begin{block}{}
 order $5$ の McGregorグラフが与えられた時に%%% 問題文の追加
\end{block}

\begin{alertblock}{}
 \begin{itemize}
  \item 12，24，33，50の4頂点が多色で彩色できる．
  \item $2^4=$\alert{\textbf{16}}通りの彩色の組み合わせを表している．%%% もっと詳細に
 \end{itemize}
\end{alertblock}
...
%          \item McGregorグラフを表すベンチマーク問題(計138問)を新たに作成．
	  ...
	  \item グラフ彩色判定問題の実行可能解の全解列挙．%%% 未定
         \end{itemize}
   \item \structure{グラフ彩色問題の実行可能解の圧縮についての調査}
         \begin{itemize}
...
%%% 大幅な変更
 \begin{enumerate}
  ...
  \item \alert{minimize符号化}:
        \begin{itemize}
         \item color符号化をベースに，同色頂点数最小化問題の目的関数を%%% もっと詳細の説明でもいいかも
               ASPの最小化関数を用いて簡潔に記述．
        \end{itemize}
  ...
  \item \alert{mult符号化}:
        \begin{itemize}
         \item color符号化をベースに，%%% color符号化はベースとしていいの?minimizeとmaximizeの「ベースに」と同義?
               頂点を彩色する色の数をASPの個数制約を用いて変更．%%% 変更よりも定義
...
\begin{frame}{実験結果:同色頂点数最大化問題}
 ...
 \begin{block}{}
  \structure{問題数}: order3$\sim$38までの36問%%% 最小化と同じ形式で
 \end{block}
...
\begin{frame}{実験結果:多色頂点数最大化問題}
 ...
 \begin{block}{}
  \structure{問題数}: order3$\sim$15までの13問%%% 最小化と同じ形式で
 \end{block}
 ...
 \begin{itemize}
  \item BB法とUSC法ではUSC法の方が良い結果を得られた．%%% (BB法が優れていた問題は何問....)
  \item USC法が10問について最適値を求めることができた．
  \item 新たに2問について最適値を発見することができた．%%% 17, 18...
 \end{itemize}
