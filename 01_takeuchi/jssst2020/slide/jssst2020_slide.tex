...
\begin{frame}{基本符号化の改良点}
 ...
 \uncover<2>{
  \begin{itemize}
   \item 範囲制約及び装備タイプが必須かどうかの別を考慮することで，
	 IWR値の総和の上下限を厳密にする．
  \end{itemize}
 }
...
\end{exampleblock}
 \begin{itemize}
  \item アトム\scode{lb_iwr(LB)}, \scode{ub_iwr(UB)}\footnotemark[2]は，
	IWR値の下限が\code{LB}，上限が\code{UB}であることを表す．
  % \item アトム\scode{lb_iwr(LB)}は，下限値が各必須タイプが選択可能なオプション
  % 	のIWR値の最小値の総和が\code{LB}であることを表す．
  % \item アトム\scode{ub_iwr(UB)}は，上限値が各タイプが選択可能なオプション
  % 	のIWR値の最大値の総和が\code{UB}であることを表す．
 \end{itemize}
 \begin{block}{}
  ...
  \end{itemize}
 \end{block}
 \footnotetext[2]{アトム\code{lb_iwr(LB), ub_iwr(UB)}はそれぞれルール2個で求めることができる．}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{実行実験}
 ...
 \begin{block}{まとめ}
  \begin{itemize}
   \item CAFE問題に対して解集合プログラミングを初めて適用した．
	 \begin{itemize}
	  \item 3種類のASP符号化によるCAFE問題ソルバーを考案
	  \item 記述性の高さ，柔軟な最適解探索などの利点を確認できた．
	 \end{itemize}
   \item 企業から提供された実データを用いた評価実験を行った．
	 \begin{itemize}
	  \item 実用規模の問題に対して適用可能であることを確認できた．
	  \item 大規模な問題に対する改良符号化の優位性を確認できた．
	 \end{itemize}
  \end{itemize}
 \end{block}
 \begin{alertblock}{今後の課題}
  \begin{itemize}
   \item 様々な制約の調査・整理・実装をすることで，CAFE問題ソルバーの拡張をする．
   \item より一般的な，可変性モデルで記述できる問題へのASPの適用
  \end{itemize}
 \end{alertblock}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
\begin{frame}[fragile]{拡張符号化}
 ...
 \begin{itemize}
  \item アトム\scode{used_v(V)}は，オプション\code{V}がいずれかの装備仕様において
	実装されたことを意味する．
  \item 目的関数中の\code{@}の右側は優先度で，予想販売台数の最大化(優先度2)，オプション数最小化(優先度1)
	の順に最適化される．
 \end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
