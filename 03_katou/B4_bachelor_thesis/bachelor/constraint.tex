...
この制約モデルを$n=3,k=2$で実行すると，コード~\ref{code:3-2_model2}のように変数と節が生成される．なお，\code{(int } \code{r_1 0 } \code{2)}は\code{r_1}が0,1,2のいずれかの値をとる整数変数であることを表す．
\lstinputlisting[float=ht,caption={%
部分和制約モデルの実行例($n=3,k=2$)},%
