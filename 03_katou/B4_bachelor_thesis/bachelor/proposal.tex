...
以降の符号化において，アトム\code{n/1}は$Q_n$の列番号，行番号を，\code{u/1}は右上がり対角線の番号を，\code{d/1}は右下がり対角線の番号を表す．アトム\code{q/2}はクイーンの有無を表す．
...
具体的には，\begin{itemize}
	     \item 5行目から8行目でマス\code{(X,Y)}にクイーンが置かれていたらその行，列，対角線方向は\code{attackable}であるという推論
	     \item 10行目で一貫性制約を用いて各マス\code{(i,j)}に対する被覆制約
	    \end{itemize}
...
\begin{itemize}
 ...
 \item 12行目から15行目でその行，列，対角線方向が\code{attackable}ならばクイーンがひとつ以上存在すること
