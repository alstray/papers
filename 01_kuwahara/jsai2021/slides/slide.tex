...
\begin{frame}{研究概要}
  ...
  \begin{itemize}
  \item \structure{方針}:
    メタ戦略の一種である\structure{巨大近傍探索 (LNS)}~[Pisinger '10]
...
\begin{block}{}\centering
  ASPソルバー{\clingo}の python インターフェースを用いて実装した．
\end{block}
\vfill
  \thicklines
...
\begin{frame}{他のアプローチとの比較}
  ...
  \end{center}
  \begin{itemize}
      \item 既知の最良値との比を，+437\% から +26\% に\alert{\bf 大幅に改善}
      \item \textsf{comp09}について，\alert{\bf 既知の最良値を更新}することに成功
  \end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{追加実験1 (予稿未記載，参考資料)}
  \begin{itemize}
  ...
  \item 新たに，\textsf{test4}について，\alert{\bf 既知の最良値を更新}することに成功
  \end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{追加実験2 (予稿未記載，参考資料)}
  \begin{itemize}
  \item CB-CTTベンチマーク問題
    \begin{itemize}
    \item 国際時間割競技会ITC-2007の問題(全21問)
...
\begin{frame}{まとめと今後の課題}
  ...
  \item \structure{優先度付き巨大近傍探索 (LNPS) を提案}
    ...
    \item 系統的探索の長所(最適性の保証)と確率的局所探索の長所(解精度)
      の両方を活かした探索
    \end{itemize}
  \item \structure{LNPS を ASP ソルバー \textit{clingo} 上に実装}
    \begin{itemize}
    \item ASPシステム{\clingo}のマルチショット ASP 解法
      と\textsf{\#heuristic}文を用いて簡潔に実装
    \end{itemize}
  \item \structure{カリキュラムベース・コース時間割問題を用いた評価実験}
    \begin{itemize}
      \item ソフト制約が多い問題集 (UD5) に対して，
        既知の最良値との比を，+437\% から +26\% に\alert{\bf 大幅に改善}
      \item \textsf{comp09}と\textsf{test4}の2問について，
